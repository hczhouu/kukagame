#include "HomeBannerModel.h"
#include <QCoreApplication>


HomeBannerModel::HomeBannerModel(QObject *parent)
    : QAbstractListModel(parent)
{}

int HomeBannerModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
...
QVariant HomeBannerModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
QHash<int, QByteArray> HomeBannerModel::roleNames() const
{
...
void HomeBannerModel::updateBannerList(const QJsonArray& arrList)
{
    beginResetModel();
