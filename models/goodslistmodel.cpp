...
GoodsListModel* GoodsListModel::pGoodsList = nullptr;

GoodsListModel::GoodsListModel(QObject *parent)
...
int GoodsListModel::rowCount(const QModelIndex &parent) const
{
...
QVariant GoodsListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
QHash<int,QByteArray> GoodsListModel::roleNames() const
{
...
void GoodsListModel::updateGoodsList(const QJsonArray& goodsList)
{
    m_isDataReady = false;
