...
window.playerRTMP = function (eventEmitter, zIndex) {
    var gEventEmitter = eventEmitter;
    var applyGameWrap = null;
    var gVideoPlayer = null;
    var gAudioPlayer = null;
    var playerPath = _getSDKPath("flowplayer.js");
    ...
    function _loadStyle() {
        ...
        link.href = playerPath + "flowplayer.css";
        link.rel = "stylesheet";
        document.head.appendChild(link);
    }

    function _getSDKPath(fileName) {
        var fileNamePath = "";
        var scripts = document.getElementsByTagName("script");
        if (scripts && scripts.length > 0) {
            for (var key in scripts) {
                var path = scripts[key].src;
                var index = path && path.indexOf(fileName);
                if (index > 0) {
                    fileNamePath = path.slice(0, index);
                    break;
                }
            }
        }
    ...
    // 创建播放器
    function _create(oContainer, width, height) {
        //创建视频播放器
        applyGameWrap = oContainer.id;
        var video_wrap = applyGameWrap + "_video_box";
        if (document.querySelector(".HMplayerBox")) {
            // if(isIEBrowser()){
            //     document.querySelector(".HMplayerBox").removeNode(true);
            //     document.querySelector("#"+audio_wrap).removeNode(true);
            //
            // }else{
            //     document.querySelector(".HMplayerBox").remove();
            //     document.querySelector("#"+audio_wrap).remove();
            // }
        }
        var oVideoElem = document.createElement("div");
        oVideoElem.setAttribute("id", video_wrap);
        oVideoElem.setAttribute("class", "HMplayerBox");

        oVideoElem.style.width = "100%";
        oVideoElem.style.height = "100%";
        oVideoElem.style.zIndex = zIndex;
        oContainer.insertBefore(oVideoElem, oContainer.childNodes[0]);

        //创建音频播放器
        var audio_wrap = applyGameWrap + "_audio_box";
        var oAudioElem = document.createElement("div");
        oAudioElem.setAttribute("id", audio_wrap);
        oAudioElem.setAttribute("style", "display:block;width:400px; height: 400px;position:absolute; z-index:-1;left:0; top:0;opacity:0.005;");

        oVideoElem.appendChild(oAudioElem);
        return {
            canvasId: "", videoId: ""
        };
    }
    ...
    function _flowplayerConfig() {
        ...
        return {
            autoplay: true, //自动播放
            //ratio: 0.5625,   //画面比例,不传的话，会默认是流的比例
            live: true,
            keyboard: false,//键盘操作
            // swfHls: playerPath + "flowplayerhls.swf?version=" + (new Date().getTime()),
            swf: playerPath + "flowplayer.swf?version=" + (new Date().getTime()),
            seedable: false,
            embed: false,
            debug: false,
            fullscreen: true,
    ...
    // 初始化视频
    function _initVideoPlayer(options, setDataInfo) {
        console.log("%c FLOWPLAYER options: ", "background:#1a73e8;color:white", options);
        //初始化配置信息

        var onFrameDelayEventCallBack = function (delay, delaySinceLastCheck, dropFrameRate, bytesPerSecond, maxBytesPerSecond) {
            gEventEmitter.emit("RTMP", "autoSwitchBitRate", {
                delay: delay,
                delaySinceLastCheck: delaySinceLastCheck,
        ...
        /**
         * 固定周期调用这个function，发送帧延迟数据
         * descriptioin: 每固定周期帧延迟结果上报
         * @param delay_ms Number 平均帧延迟(毫秒)
         * @param max_delay_ms Number 最大帧延迟(毫秒)
         * @param min_delay_ts Number 最大帧延迟时间戳(毫秒)
         * @param min_delay_ms Number 最小帧延迟(毫秒)
         * @param min_delay_ts Number 最小帧延迟时间戳(毫秒)
         */
        var sendFrameDelayCountly = function (delay_ms, max_delay_ms, max_delay_ts, min_delay_ms, min_delay_ts, detailInfo) {
            gEventEmitter.emit("RTMP", "sendFrameDelayCountly", {
                delay_ms: delay_ms,
                max_delay_ms: max_delay_ms,
                max_delay_ts: max_delay_ts,
                min_delay_ms: min_delay_ms,
                min_delay_ts: min_delay_ts,
                detailInfo: detailInfo
            });
        };
        var frameDelayDetectorOptions = {
            durationInSecond: setDataInfo.switchBR_duration,
            detectIntervalInMili: setDataInfo.detect_interval || 500,
            triggerPlayTimeDelayInMili: setDataInfo.play_time_delay || 600,
            frozenTimeInSecond: setDataInfo.switchBR_frozentime,
            frameDelayInfoReportIntervelInSecond: setDataInfo.frameDelayInfo_interval || -1,
            isReportFrameDelayDetailInfo: setDataInfo.frameDelayInfo_isReportDetailInfo || 0,
            triggerDelayTimeSinceLastCheckInMili: setDataInfo.frameDelayInfo_triggerDelayTimeSinceLastCheck || 150,
            triggerDropFrameRate: setDataInfo.frameDelayInfo_triggerDropFrameRate || 100,
            startDetectDelayTimeInSecond: setDataInfo.frameDelayStartDelayTime || 5,
            callback: onFrameDelayEventCallBack,
            reportCallback: sendFrameDelayCountly,
        };

        var conf = _flowplayerConfig();
        Object.assign(flowplayer.conf, conf, options);

        var sources = [{
            type: "video/flash",
            src: flowplayer.conf.videoUrl,
            customRotate: flowplayer.conf.rotate,
            frameDelayDetector: frameDelayDetectorOptions,
            logTag: "video_player",
        }];

        if (gVideoPlayer) {
            gVideoPlayer.stop(true);
            if (gVideoPlayer.load({sources: sources}) === false) {
                gEventEmitter.emit("RTMP", "video_error");
            }
        } else {
            gVideoPlayer = flowplayer("#" + applyGameWrap + "_video_box", {
                keyboard: false,
                clip: {
                    provider: "rtmp",
                    sources: sources
    ...
    // 初始化音频
    function _initAndStartAudioPlayer() {
        var audio_wrap = "#" + applyGameWrap + "_audio_box";
        var sources = [{
            type: "video/flash",
            src: flowplayer.conf.audioUrl,
            logTag: "audio_player",
        }];

        if (gAudioPlayer) {
            gAudioPlayer.stop();
            gAudioPlayer.load({sources: sources});
        } else {
            gAudioPlayer = flowplayer(audio_wrap, {
                rtmp: "",
                ratio: 1 / 1000,
                clip: {
                    provider: "rtmp",
        ...
        if (gAudioPlayer.mute) {
            gAudioPlayer.mute(true);
        }
    }

    function _initLoadEvent() {

        // 视频失败
        gVideoPlayer.off("error").on("error", function (e, api, error) {
            console.log("%c FLOWPLAYER gVideoPlayer error: ", "background:#1a73e8;color:white", error);
            e.stopPropagation();
            gEventEmitter.emit("RTMP", "video_error", error);
        });

        //视频看到第一帧
        gVideoPlayer.one("ready", function () {
            gEventEmitter.emit("RTMP", "video_ready");
        });

        // 第一帧展示
        gVideoPlayer.one("visibleFrameReady", function () {
            gEventEmitter.emit("RTMP", "play");
        });

        //视频流链接失败
        gVideoPlayer.one("disconnect", function () {
            gEventEmitter.emit("RTMP", "video_connect_failed");
        });

        //视频流连接成功
        gVideoPlayer.one("connected", function () {
            gEventEmitter.emit("RTMP", "video_connected");
        });

        //flash不可用
        gVideoPlayer.off("flashdisabled").on("flashdisabled", function () {
            gEventEmitter.emit("RTMP", "flash_disabled");
        });

        // 音频失败
        gAudioPlayer.off("error").on("error", function (d1, d2, error) {
            gEventEmitter.emit("RTMP", "audio_error", error);
        });

        // 音频链接成功
        gAudioPlayer.off("connected").one("connected", function () {
            gEventEmitter.emit("RTMP", "audio_connected");
        });
    }

    // 开始
    function _connect(options, setDataInfo) {
        console.log("%c FLOWPLAYER : ", "background:#1a73e8;color:white", options, setDataInfo);
        gEventEmitter.emit("RTMP", "video_connect", options.videoUrl);
        _initVideoPlayer(options, setDataInfo);
    }

    // 停止
    function _disconnect() {
        if (gVideoPlayer) {
            gVideoPlayer.stop && gVideoPlayer.stop(true);

            gVideoPlayer.off("error");
            gVideoPlayer.off("ready");
            gVideoPlayer.off("visibleFrameReady");
            gVideoPlayer.off("disconnect");
            gVideoPlayer.off("connected");
            gVideoPlayer.off("flashdisabled");

            gVideoPlayer = null;
        }

        if (gAudioPlayer) {
            gAudioPlayer.stop && gAudioPlayer.stop();

            gAudioPlayer.off("error");
            gAudioPlayer.off("connected");
            gAudioPlayer = null;
        }
    }

    function _enableAudio(enabled) {
        if (!gAudioPlayer) {
            _initAndStartAudioPlayer();
        }
        if (gAudioPlayer) {
            gAudioPlayer.mute(enabled);
            return true;
        }
        return false;
    }
...
var sgFlashInputEventListener = {keyup: null, keydown: null, mousedown: null, mouseup: null, mousemove: null};
// console.log("sgFlashInputEventListener is init .....") ;
    ...
    return (function e(t, n, r) {
        ...
        2: [function (_dereq_, module, exports) {
            ...
            module.exports = function embed(swf, flashvars, wmode, bgColor) {
                wmode = wmode || "opaque";  // direct, opaque, window

                // console.log("flash wmode: " + wmode)
        ...
        3: [function (_dereq_, module, exports) {
            ...
            engineImpl = function flashEngine(player, root) {
                ...
                var triggerDelayEventCallbackId;
                var triggerReportFrameDelayCallbackId;
                ...
                var engine = {
                    ...
                    load: function (video) {
                        ...
                        if (typeof (video.captureInputEvent) == "undefined")
                            video.captureInputEvent = false;

                        if (typeof (video.isSplashStream) == "undefined")
                            video.isSplashStream = false;

                        if (api) {
                            ["live", "preload", "loop", "captureInputEvent", "customRotate"].forEach(function (prop) {
                            ...
                            var triggerDelayEventFun;
                            var triggerReportFrameDelayInfoFun;

                            if (typeof (video.frameDelayDetector) != "undefined") {

                                ["durationInSecond", "detectIntervalInMili", "triggerPlayTimeDelayInMili", "frozenTimeInSecond",
                                    "frameDelayInfoReportIntervelInSecond", "isReportFrameDelayDetailInfo", "triggerDelayTimeSinceLastCheckInMili",
                                    "triggerDropFrameRate", "startDetectDelayTimeInSecond"].forEach(function (prop) {
                                    if (!video.frameDelayDetector.hasOwnProperty(prop)) return;
                                    api.__set(prop, video.frameDelayDetector[prop]);
                                });

                                if (!triggerDelayEventCallbackId) {
                                    triggerDelayEventCallbackId = "hmTriggerDelayEventCallback" + ("" + Math.random()).slice(3, 15);
                                }

                                triggerDelayEventFun = video.frameDelayDetector.callback;
                                window[triggerDelayEventCallbackId] = triggerDelayEventFun;
                                api.__set("triggerCallbackId", triggerDelayEventCallbackId);

                                if (!triggerReportFrameDelayCallbackId)
                                    triggerReportFrameDelayCallbackId = "hmTriggerReportFrameDelayCallback" + ("" + Math.random()).slice(3, 15);

                                triggerReportFrameDelayInfoFun = video.frameDelayDetector.reportCallback;
                                window[triggerReportFrameDelayCallbackId] = triggerReportFrameDelayInfoFun;
                                api.__set("triggerReportFrameDelayCallbackId", triggerReportFrameDelayCallbackId);
                            }

                            var oldSplashVal = api.__get("isSplashStream");
                            api.__set("isSplashStream", video.isSplashStream);

                            api.__play(url, true);

                            /*
             if (video.isSplashStream.toString() != oldSplashVal.toString()) {
                 api.__play(url, true);
             } else {
                 api.__play(url, providerChangeNeeded || video.rtmp && video.rtmp !== conf.rtmp);
                            ...
                            // console.log("flowplayer play with url: " + url, ", captureInputEvent: " + video.captureInputEvent) ;
                            ...
                            var opts = {
                                ...
                                url: url,
                                captureInputEvent: video.captureInputEvent,
                                customRotate: video.customRotate,
                                isSplashStream: video.isSplashStream,
                                callback: callbackId
                            };
                            ...
                            if (typeof (video.frameDelayDetector) != "undefined") {
                                ...
                                opts.durationInSecond = video.frameDelayDetector.durationInSecond;
                                opts.detectIntervalInMili = video.frameDelayDetector.detectIntervalInMili;
                                opts.triggerPlayTimeDelayInMili = video.frameDelayDetector.triggerPlayTimeDelayInMili;
                                opts.startDetectDelayTimeInSecond = video.frameDelayDetector.startDetectDelayTimeInSecond;
                                opts.frozenTimeInSecond = video.frameDelayDetector.frozenTimeInSecond;
                                opts.frameDelayInfoReportIntervelInSecond = video.frameDelayDetector.frameDelayInfoReportIntervelInSecond;
                                opts.isReportFrameDelayDetailInfo = video.frameDelayDetector.isReportFrameDelayDetailInfo;
                                opts.triggerDelayTimeSinceLastCheckInMili = video.frameDelayDetector.triggerDelayTimeSinceLastCheckInMili;
                                opts.triggerDropFrameRate = video.frameDelayDetector.triggerDropFrameRate;

                                triggerDelayEventCallbackId = "hmTriggerDelayEventCallback" + ("" + Math.random()).slice(3, 15);
                                opts.triggerCallbackId = triggerDelayEventCallbackId;
                                triggerDelayEventFun = video.frameDelayDetector.callback;

                                triggerReportFrameDelayCallbackId = "hmTriggerReportFrameDelayCallback" + ("" + Math.random()).slice(3, 15);
                                opts.triggerReportFrameDelayCallbackId = triggerReportFrameDelayCallbackId;
                                triggerReportFrameDelayInfoFun = video.frameDelayDetector.reportCallback;
                            }

                            // optional conf
                            ["proxy", "key", "autoplay", "preload", "subscribe", "live", "rotate", "loop", "debug", "splash", "poster", "rtmpt"].forEach(function (key) {
                            ...
                            opts.debug = window.FLOWPLAYER_LOG;
                            ...
                            var logTag = video.logTag;
                            if (!logTag) {
                                logTag = "flash_player";
                            }

                            // listen
                            window[callbackId] = function (type, arg, argE) {
                                var video = loadVideo;
                                ...
                                if (window.FLOWPLAYER_LOG) {

                                    if (type != "mousedown" && type != "mouseup" && type != "mousemove") {
                                        console.log(logTag + " --------------: " + type + ", " + arg + ", " + argE);
                                    }
                                }
                                ...
                                switch (type) {
                                    ...
                                    case "click":
                                        ...
                                        break;
                                    case "keydown":
                                        if (sgFlashInputEventListener.keydown) {
                                            sgFlashInputEventListener.keydown(arg);
                                        }
                                        return;

                                    case "keyup":
                                        if (sgFlashInputEventListener.keyup) {
                                            sgFlashInputEventListener.keyup(arg);
                                        }
                                        return;

                                    case "mousedown":
                                        if (sgFlashInputEventListener.mousedown) {
                                            sgFlashInputEventListener.mousedown(arg, argE);
                                        }
                                        return;

                                    case "mouseup":
                                        if (sgFlashInputEventListener.mouseup) {
                                            sgFlashInputEventListener.mouseup(arg, argE);
                                        }
                                        return;

                                    case "mousemove":
                                        if (sgFlashInputEventListener.mousemove) {
                                            sgFlashInputEventListener.mousemove(arg, argE);
                                        }
                                        return;
                 ...
                 bean.fire(root, type, [event]);
                    ...
                    unload: function () {
                        ...
                        try {
                            ...
                            if (triggerDelayEventCallbackId && window[triggerDelayEventCallbackId]) delete window[triggerDelayEventCallbackId];
                            if (triggerReportFrameDelayCallbackId && window[triggerReportFrameDelayCallbackId]) delete window[triggerReportFrameDelayCallbackId];
                        } catch (e) {
                        }
                    ...
                    left: function () {
                        return api.__get("hm_left");
                    },

                    top: function () {
                        return api.__get("hm_top");
                    }
        ...
        17: [function (_dereq_, module, exports) {
         ...
         <div class=\"message\"><h2></h2><p></p></div>".replace(/class="/g, "class=\"fp-"));
                ...
                api.on("ready", function (ev, api, video) {
                    ...
                    //CloudSaas-154 stop show error @zhaofusen
                    return true;
                    ...
                    if (error) {
                        ...
                        console.warn("flowplayer got error: " + error.message);
                        api.error = true;
   ...
   if (has_bg && !conf.splash) {
      ...
      api.on("ready stop", function() {
                ...
                var api = {
                    ...
                    load: function (video, callback) {
                        ...
                        if (api.error || api.loading) {
                            ...
                            if (api.error)
                                console.warn("flowplayer load failed for player is in error status");

                            if (api.loading)
                                console.warn("flowplayer load failed for player is loading");
                    ...
                    pause: function (fn, isStayInLastFrame) {
                        if (api.ready && !api.loading) {
                            engine.pause(isStayInLastFrame);
                            api.one("pause", fn);
                        }
                        return api;
                    },
         ...
         */
                    ...
                    stop: function (isStayInLastFrame, isNotCheckReady) {
                        if (isNotCheckReady || api.ready) {
                            api.pause(undefined, isStayInLastFrame);
                            api.seek(0, function () {
                    ...
                    left: function () {
                        ...
                        var lefVal = engine.left();
                        if (!lefVal) {
                            return 0;
                        }

                        return parseInt(lefVal, 10);
                    },
                    ...
                    top: function () {
                        ...
                        var topVal = engine.top();
                        if (!topVal) {
                            return 0;
                        }

                        return parseInt(topVal, 10);
                    }
                ...
                if (!root.getAttribute("data-flowplayer-instance-id")) { // Only bind once
                    ...
                    }).on("ready pause pauseInLastFrame resume unload finish stop", function (e, _api, video) {
