...
//获取商城页面Bannner
void HomePage::getGameShopBanner(GameShopBannerModel* shopBannerModel,
                                 GameShopTabModel* shopTabModel)
{
    connect(this, &HomePage::updateGameShopBannerList, shopBannerModel,
            &GameShopBannerModel::updateGameShopBannerList, Qt::UniqueConnection);

    connect(this, &HomePage::updateShopTab, shopTabModel,
            &GameShopTabModel::updateShopTabTitle, Qt::UniqueConnection);

    std::thread([=](){
        m_shopBannerIsReady = false;
        emit shopBannerIsReadyChanged();

        std::string resp;
        QJsonObject jsonData;
        jsonData.insert("clientType", 101);
        jsonData.insert("pageNo", 1);
        jsonData.insert("pageSize", 10);
        QJsonDocument docJson;
        docJson.setObject(jsonData);
        ...
        if (!CommonFunc::SendHttpRequest(resp, true, url, postData, ""))
        {
            LOG(ERROR) << "get msglist error " << resp;
            if (HttpClient::getInstance() != nullptr)
            {
                emit HttpClient::getInstance()->showMsgPopup(true, QByteArray::fromStdString(resp));
            }
        ...
        if (itemData.at(0).isArray())
        {
            emit updateGameShopBannerList(itemData.at(0).toArray());
            m_shopBannerIsReady = true;
            emit shopBannerIsReadyChanged();
        } else {
            LOG(ERROR) << "gameshop top banner is empty";
        }
...
//获取商城页面标签页
void HomePage::getShopGameItems()
{
    std::thread([=](){
        m_shopGoodModelIsReady = false;
        emit shopGoodModelIsReadyChanged();

        std::string resp;
        QJsonObject jsonData;
        jsonData.insert("scope", "mall");
        QJsonDocument docJson;
        docJson.setObject(jsonData);
        std::string postData = docJson.toJson().toStdString();
        std::string url = CommonFunc::GetBaseUrl().toStdString();
        url.append("pc/mall/selectGoodsV2");
        ...
        emit updateShopTab(itemData);
        m_shopGoodModelIsReady = true;
        emit shopGoodModelIsReadyChanged();
