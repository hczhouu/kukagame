#include "ActivitiesNoticeModel.h"

ActivitiesNoticeModel::ActivitiesNoticeModel(QObject *parent)
    : QAbstractListModel(parent)
{}

int ActivitiesNoticeModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
...
QVariant ActivitiesNoticeModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
QHash<int,QByteArray> ActivitiesNoticeModel::roleNames() const
{
...
void ActivitiesNoticeModel::updateActivitiesList(const QJsonArray& arrList)
{
    beginResetModel();
